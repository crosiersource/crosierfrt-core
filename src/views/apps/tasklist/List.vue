  ...
  <ul class="list-none p-0 m-0">
    <li v-for="(task, i) in taskList" :key="task" class="flex flex-col gap-4 md:flex-row md:items-center p-2 border-b border-surface-200 dark:border-surface-700">
      <div class="flex items-center flex-1">
        <Checkbox v-model="task.completed" binary :input-id="task.id?.toString()" @change="onCheckboxChange(task)" />
      ...
      <div class="flex flex-1 gap-4 flex-col sm:flex-row sm:justify-between">
        <div class="flex items-center">
          ...
          <span v-if="task.startDate" class="flex items-center font-semibold whitespace-nowrap"><i class="pi pi-clock mr-2" />{{ parseDate(task.startDate) }}</span>
        </div>
        <div class="flex items-center sm:justify-end">
          <div class="mr-4">
            <AvatarGroup v-if="task.members?.length > 0">
              ...
              <Avatar v-if="task && task.members && task.members.length > 4" :label="`+${task.members.length - 4}`" shape="circle" size="large" class="bg-blue-500" :style="{ color: '#212121', border: '2px solid var(--surface-border)' }" />
            </AvatarGroup>
          </div>
