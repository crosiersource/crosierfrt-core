            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    ...
                    <div>
                        ...
                        <InputText id="email1" v-model="authStore.username" type="text" placeholder="Nome de usuário" class="w-full md:w-[30rem] mb-8" />

                        <label for="password1" class="block text-surface-900 dark:text-surface-0 font-medium text-xl mb-2">Senha</label>
                        <Password id="password1" v-model="password" placeholder="Digite sua senha" :toggle-mask="true" class="mb-4" fluid :feedback="false" />
                        ...
                        <Button type="button" label="Entrar" class="w-full" @click="login()" />
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        login() {
            // console.log('tem router?', this.$router);
